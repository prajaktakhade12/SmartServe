...
class _DashboardScreenState extends State<DashboardScreen> {
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text(
                      isHead ? 'All Categories Overview'
                          : '${_formatCategory(category)} Department Overview',
                ...
                // Stats Cards
                Row(children: [
                  _statCard('Total Issues', _data['total'] ?? 0,
                      Icons.list_alt_rounded, AppTheme.primary, Colors.blue.shade50),
                  const SizedBox(width: 16),
                  _statCard('Reported', _data['reported'] ?? 0,
                      Icons.flag_rounded, AppTheme.reported, Colors.orange.shade50),
                  const SizedBox(width: 16),
                  _statCard('In Progress', _data['in_progress'] ?? 0,
                      Icons.pending_rounded, AppTheme.inProgress, Colors.amber.shade50),
                  const SizedBox(width: 16),
                  _statCard('Completed', _data['completed'] ?? 0,
                      Icons.check_circle_rounded, AppTheme.completed, Colors.green.shade50),
                ]),
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Row(children: [
                        Expanded(child: _progressBar(
                          'Reported', _data['reported'] ?? 0,
                          _data['total'] ?? 1, AppTheme.reported)),
                        const SizedBox(width: 24),
                        Expanded(child: _progressBar(
                          'In Progress', _data['in_progress'] ?? 0,
                          _data['total'] ?? 1, AppTheme.inProgress)),
                        const SizedBox(width: 24),
                        Expanded(child: _progressBar(
                          'Completed', _data['completed'] ?? 0,
                          _data['total'] ?? 1, AppTheme.completed)),
                      ]),
                    ]),
                  ),
                ),
  ...
  Widget _progressBar(String label, int value, int total, Color color) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 4),
      Text('$value issues', style: TextStyle(fontSize: 11,
